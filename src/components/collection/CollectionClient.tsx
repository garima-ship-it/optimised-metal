...
function ProductCard({ card }: { card: Card }) {
  ...
  return (
    <Link href={`/product/${card.id}`} style={{ textDecoration: 'none', cursor: 'pointer', display: 'block' }}>
      <div>
        <div className="product-card-img" style={{ width: 320, height: 220, borderRadius: 14, overflow: 'hidden', position: 'relative', transition: 'transform .2s' }}
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 38, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 10px' }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: 'white', textTransform: 'uppercase', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '60%' }}>{card.name}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 700, color: 'white' }}>₹{card.dprice ?? card.price}</span>
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 0 4px', flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 12, color: '#a8a5a5' }}>({reviewCount})</span>
  ...
  return (
    <>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="collection-grid" style={{ padding: '32px 60px 60px', display: 'grid', gridTemplateColumns: 'repeat(4, 320px)', gap: 20 }}>
